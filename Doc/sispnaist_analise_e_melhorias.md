...
✅ **Concluído** — Implementado um sistema de alertas configurável para eventos críticos (e.g., aumento súbito de acidentes, vacinações vencendo, não conformidades, monitoramento crítico de trabalhadores). Canais integrados: painel de alertas dedicado em `/alertas` (com sino de notificações no cabeçalho) e e-mail automático. A detecção é feita por agendamento (`node-cron`) com regras configuráveis (`AlertasRegras`) e parâmetros do sistema, respeitando o escopo: administradores recebem todos os alertas e gestores apenas os da sua empresa.
...
*   **Autenticação de Dois Fatores (2FA) ✅Concluído**: Adicionar 2FA para aumentar a segurança das contas de usuário, especialmente para perfis de administrador e gestor.
