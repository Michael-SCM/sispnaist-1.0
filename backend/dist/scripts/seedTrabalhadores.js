...
exports.seedTrabalhadores = seedTrabalhadores;
...
const Questionario_js_1 = __importDefault(require("../models/Questionario.js"));
const Catalogo_js_1 = __importDefault(require("../models/Catalogo.js"));
...
async function seedTrabalhadores(targetCount = 2000) {
    ...
    await Empresa_js_1.default.deleteMany({});
    await Unidade_js_1.default.deleteMany({});
    const empresasDados = [
    ...
    for (let i = 0; i < empresasDados.length; i++) {
        ...
        const emp = await Empresa_js_1.default.create({
    ...
    for (let e = 0; e < empresas.length; e++) {
        ...
        for (let u = 0; u < numUnidades; u++) {
            ...
            const unidade = await Unidade_js_1.default.create({
                nome: nomeUnidade,
    ...
    // Questionário
    let questionario = await Questionario_js_1.default.findOne();
    if (!questionario) {
        console.log('📝 Nenhum questionário encontrado. Criando...');
        questionario = await Questionario_js_1.default.create({
    ...
    const catalogos = await Catalogo_js_1.default.find({ ativo: true });
    ...
    await Trabalhador_js_1.default.deleteMany({});
    await TrabalhadorAfastamento_js_1.default.deleteMany({});
    await TrabalhadorDependente_js_1.default.deleteMany({});
    await TrabalhadorInformacao_js_1.default.deleteMany({});
    await TrabalhadorProcessoTrabalho_js_1.default.deleteMany({});
    await TrabalhadorReadaptacao_js_1.default.deleteMany({});
    await TrabalhadorVinculo_js_1.default.deleteMany({});
    ...
    for (let i = 1; i <= targetCount; i++) {
        ...
        const workerId = new mongoose_1.default.Types.ObjectId();
        // ---- NOME ----
        ...
        const jornadaStr = (() => {
            const r = rand();
            if (r < 0.50)
                return '40 horas semanais';
            if (r < 0.80)
                return '44 horas semanais';
            if (r < 0.93)
                return '30 horas semanais';
        ...
        const funcaoStr = (() => {
            ...
            if (r < 0.45)
                return 'Enfermeiro(a)';
            if (r < 0.70)
                return 'Técnico(a) de Enfermagem';
            if (r < 0.88)
        ...
        const salario = (() => {
            if (funcaoStr === 'Médico(a)')
                return 12000 + Math.floor(rand() * 8000);
            if (funcaoStr === 'Enfermeiro(a)')
                return 5000 + Math.floor(rand() * 3000);
            if (funcaoStr === 'Técnico(a) de Enfermagem')
                return 2500 + Math.floor(rand() * 1500);
            if (funcaoStr === 'Administrativo')
                return 2200 + Math.floor(rand() * 1800);
    ...
    await Trabalhador_js_1.default.insertMany(workersToInsert);
    console.log('  ▸ Afastamentos...');
    if (afastamentosToInsert.length > 0)
        await TrabalhadorAfastamento_js_1.default.insertMany(afastamentosToInsert);
    console.log('  ▸ Dependentes...');
    if (dependentesToInsert.length > 0)
        await TrabalhadorDependente_js_1.default.insertMany(dependentesToInsert);
    console.log('  ▸ Informações de Saúde...');
    await TrabalhadorInformacao_js_1.default.insertMany(informacoesToInsert);
    console.log('  ▸ Ocorrências de Violência...');
    if (ocorrenciasToInsert.length > 0)
        await TrabalhadorOcorrenciaViolencia_js_1.default.insertMany(ocorrenciasToInsert);
    console.log('  ▸ Processos de Trabalho...');
    await TrabalhadorProcessoTrabalho_js_1.default.insertMany(processosToInsert);
    console.log('  ▸ Readaptações...');
    if (readaptacoesToInsert.length > 0)
        await TrabalhadorReadaptacao_js_1.default.insertMany(readaptacoesToInsert);
    console.log('  ▸ Vínculos...');
    await TrabalhadorVinculo_js_1.default.insertMany(vinculosToInsert);
