...
router.get('/trabalhador/:trabalhadorId', auditRead('Doenca'), doencaController.obterPorTrabalhador);

// CRUD padrão (rotas genéricas por último)
router.post('/', validateRequest(criarDoencaSchema), doencaController.criar);
router.get('/', validateQuery(listarDoencasQuerySchema), auditReadList('Doenca'), doencaController.listar);
router.get('/:id', validateObjectId('id'), auditRead('Doenca'), doencaController.obter);
