...
const config = {
  ...
  alert: {
    cronSchedule: process.env.ALERT_CRON_SCHEDULE || '0 7 * * *',
    runOnStart: process.env.ALERT_RUN_ON_START === 'true',
    diasAntecedenciaVacina: parseInt(process.env.ALERT_DIAS_VACINA || '30', 10),
    limiarPicoAcidentes: parseInt(process.env.ALERT_LIMIAR_PICO || '3', 10),
    janelaPicoDias: parseInt(process.env.ALERT_JANELA_PICO || '7', 10),
    periodoComparacaoDias: parseInt(process.env.ALERT_PERIODO_COMPARACAO || '7', 10),
  },
